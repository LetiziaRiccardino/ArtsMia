...
public class ArtsmiaDAO {
	...
	public int getPeso(ArtObject a1, ArtObject a2) {
		String sql= "SELECT COUNT(*) AS peso"
				...
				+ "WHERE e1.exhibitions_id= e2.exhibitions_id AND e1.exhibitions_id=? AND e2.exhibitions_id=? ";
		Connection conn= DBConnect.getConnection();
		try {
			PreparedStatement st = conn.prepareStatement(sql);
	...
	//mi serve una classe di supporto per salvare le info di questo metodo--> Adicenza
	public List<Adiacenza> getAdiacenze(Map<Integer, ArtObject> idMap) {
		...
		try {
			...
			ResultSet  res = st.executeQuery();
			while(res.next()) {
				result.add(new Adiacenza(idMap.get(res.getInt("e1")), idMap.get(res.getInt("e1")), res.getInt("peso")));
			}
			conn.close();
			return result;
